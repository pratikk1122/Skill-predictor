...
const StatCard = ({ target, suffix, label, sub, icon }) => (
  <div className="bg-white p-8 rounded-[2rem] border border-blue-greeny/5 shadow-sm text-center hover:shadow-lg transition-all">
    <div className="text-blue-greeny mb-4 text-2xl">
        <i className={`fas ${icon}`}></i>
    </div>
    <h3 className="text-4xl font-heading font-black text-text-dark mb-1">
      {label === "Support" ? target : <><Counter target={target} />{suffix}</>}
    </h3>
    <p className="text-lg font-bold text-blue-greeny mb-1">{label}</p>
    <p className="text-text-light text-xs">{sub}</p>
  </div>
);
...
const LandingPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white font-sans selection:bg-pinky relative">
      ...
      {/* Navigation */}
      <nav className={`fixed w-full z-50 h-20 flex items-center transition-all duration-300 ${
        isScrolled ? 'bg-white shadow-md' : 'bg-transparent'
      }`}>
        <div className="max-w-7xl mx-auto px-6 w-full flex justify-between items-center">
          <div className="flex items-center gap-2">
            <div className="w-10 h-10 bg-blue-greeny rounded-xl flex items-center justify-center shadow-lg shadow-blue-greeny/20 rotate-3">
              <i className="fas fa-graduation-cap text-white text-xl"></i>
            </div>
            <span className={`text-xl font-heading font-extrabold tracking-tight transition-colors duration-300 ${
          ...
          <div className="flex items-center gap-6">
            <Link to="/login" className={`font-bold text-sm transition-colors duration-300 hover:text-blue-greeny ${
              isScrolled ? 'text-gray-700' : 'text-white'
            }`}>Log in</Link>
            <Link to="/login?mode=signup" className="bg-blue-greeny hover:bg-blue-greeny-dark text-white px-8 py-2.5 rounded-full font-bold text-sm transition-all shadow-md">Sign up</Link>
          </div>
        </div>
      </nav>

      {/* Hero */}
      <section className="relative pt-40 pb-24 min-h-screen flex items-center justify-center text-center bg-gray-900">
        ...
        <div className="max-w-5xl mx-auto px-6 relative z-20">
          ...
          <h1 className="text-5xl md:text-6xl font-heading font-black text-white mb-8 leading-tight tracking-tight">
          ...
          <button 
            ...
            className="group relative px-12 py-5 bg-white/10 backdrop-blur-md border border-white/30 text-white rounded-full font-black uppercase tracking-[0.25em] overflow-hidden transition-all duration-300 hover:border-blue-greeny hover:shadow-[0_0_30px_rgba(45,212,191,0.5)]"
          >
      ...
      <section id="stats" className="py-24 px-6 bg-teeny-greeny">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            <div className="relative">
                ...
                <h2 className="text-4xl md:text-5xl font-heading font-black text-text-dark mb-8 leading-tight">
                    Empowering Your <br/> <span className="text-blue-greeny">Professional Journey</span>
                </h2>
                <p className="text-lg text-text-light leading-relaxed mb-8 font-medium">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                        ...
                        item.link ? (
                            <Link to={item.link} key={i} className="flex gap-4 items-start p-4 rounded-2xl hover:shadow-md transition-all border border-transparent hover:border-slate-100 cursor-pointer">
                                <div className={`w-12 h-12 ${item.bg} ${item.color} rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm`}>
                                    <i className={`fas ${item.icon} text-lg`}></i>
                                </div>
                                <div>
                                    <h4 className="font-black text-text-dark text-sm mb-1">{item.title}</h4>
                                    <p className="text-[11px] text-text-light leading-tight">{item.desc}</p>
                                </div>
                            </Link>
                        ) : (
                            <div key={i} className="flex gap-4 items-start p-4 rounded-2xl hover:shadow-md transition-all border border-transparent hover:border-slate-100">
                                <div className={`w-12 h-12 ${item.bg} ${item.color} rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm`}>
                                    <i className={`fas ${item.icon} text-lg`}></i>
                                </div>
                                <div>
            ...
            <div className="bg-teeny-greeny rounded-[3rem] p-12 relative overflow-hidden border border-blue-greeny/5 shadow-inner">
                <div className="absolute top-0 right-0 p-10 opacity-10">
                    <i className="fas fa-quote-right text-9xl text-blue-greeny"></i>
                </div>
                <h3 className="text-2xl font-bold text-blue-greeny mb-6">Our Mission</h3>
                <p className="text-xl text-text-dark font-medium leading-loose italic">
                ...
                <div className="mt-10 flex items-center gap-4">
                    <div className="w-14 h-14 bg-blue-greeny rounded-full flex items-center justify-center shadow-lg border-4 border-white">
                        <span className="text-white font-black text-xl tracking-tighter">SP</span>
                    </div>
                    <div>
                        <p className="font-black text-text-dark">Team SkillPredictor</p>
                    </div>
                </div>
            </div>
          </div>
      ...
      <section id="contact" className="py-32 bg-teeny-greeny relative overflow-hidden">
        <div className="max-w-4xl mx-auto px-6 text-center relative z-10">
            <h2 className="text-4xl font-heading font-black text-text-dark mb-4">Get in Touch</h2>
            <p className="text-text-light mb-16 font-medium">Have a question? Our Helpdesk is ready to assist you on your journey.</p>
            
            <div className="relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-blue-greeny/20 to-teal-500/20 rounded-[3.5rem] blur-xl opacity-50 group-hover:opacity-100 transition-opacity duration-500"></div>
              
              <form onSubmit={handleContactSubmit} className="relative bg-white/80 backdrop-blur-xl p-12 rounded-[3.5rem] shadow-2xl text-left border border-white/50">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
                      <div className="space-y-3">
                          ...
                          <div className="relative group/input">
                            ...
                            <input required type="text" placeholder="John Doe" className="w-full pl-12 pr-6 py-5 bg-slate-50/50 border-2 border-transparent rounded-[1.5rem] focus:border-blue-greeny/20 focus:bg-white focus:ring-4 focus:ring-blue-greeny/5 outline-none transition-all font-medium text-sm" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} />
                          </div>
                      </div>
                      <div className="space-y-3">
                          ...
                          <div className="relative group/input">
                            ...
                            <input required type="email" placeholder="john@example.com" className="w-full pl-12 pr-6 py-5 bg-slate-50/50 border-2 border-transparent rounded-[1.5rem] focus:border-blue-greeny/20 focus:bg-white focus:ring-4 focus:ring-blue-greeny/5 outline-none transition-all font-medium text-sm" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} />
                          </div>
                      </div>
                  </div>
                  <div className="space-y-3 mb-10">
                      <label className="text-[10px] font-black text-blue-greeny uppercase tracking-[0.2em] ml-2">How can we help?</label>
                      <textarea required rows="5" placeholder="Tell us about your query..." className="w-full px-8 py-5 bg-slate-50/50 border-2 border-transparent rounded-[2rem] focus:border-blue-greeny/20 focus:bg-white focus:ring-4 focus:ring-blue-greeny/5 outline-none transition-all resize-none font-medium text-sm" value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})}></textarea>
                  </div>
                  <button type="submit" disabled={loading} className="group relative w-full bg-blue-greeny text-white py-6 rounded-[1.5rem] font-black uppercase tracking-[0.3em] text-xs overflow-hidden transition-all shadow-xl shadow-blue-greeny/25 hover:shadow-blue-greeny/40 disabled:opacity-50 active:scale-[0.98]">
        ...
        <div 
          ...
          className="fixed bottom-10 right-10 w-20 h-20 bg-blue-greeny rounded-full flex items-center justify-center text-white shadow-2xl cursor-pointer hover:scale-110 active:scale-95 transition-all z-[90] group border-4 border-white"
        >
          <div className="absolute inset-0 rounded-full bg-blue-greeny animate-ping opacity-20 group-hover:opacity-40"></div>
          <i className="fas fa-robot text-3xl group-hover:rotate-12 transition-transform"></i>
          <span className="absolute -top-1 -right-1 bg-pinky text-white text-[9px] font-black px-2.5 py-1 rounded-full uppercase tracking-tighter shadow-lg border-2 border-white">Live AI</span>
        </div>

        {/* 🔥 PROFESSIONAL PREMIUM AI CHAT WIDGET */}
        {isAiOpen && (
          <div className="fixed bottom-32 right-10 z-[100] w-[400px] h-[600px] animate-in zoom-in-95 slide-in-from-bottom-10 duration-300 pointer-events-auto">
            <div className="bg-white w-full h-full rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden border border-slate-100">
              
              {/* Header: Sleek Design */}
              <div className="bg-blue-greeny p-6 flex justify-between items-center text-white shrink-0 relative overflow-hidden">
