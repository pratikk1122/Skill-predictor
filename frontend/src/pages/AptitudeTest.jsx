...
import { useParams, useNavigate, useLocation } from 'react-router-dom'; // 🔥 Requirement: Added useLocation
import axios from 'axios'; // 🔥 Requirement: Added axios for score submission
import api from '../services/api'; 
import { ROUTES } from "../routes/routes"; 

const AptitudeTest = () => {
  // 🔥 Captures "mathematics", "logical-reasoning", etc. from the URL
  const { category } = useParams();
  const navigate = useNavigate(); 
  const location = useLocation(); // 🔥 Requirement: Capture URL state

  // 🔥 NEW REQUIREMENT: Extract attemptId from URL to update real score in CompanyPrep
  const queryParams = new URLSearchParams(location.search);
  const attemptId = queryParams.get("attemptId");
  
  // Format the category for display (e.g., "logical-reasoning" -> "Logical Reasoning")
  ...
  const fetchNewQuestion = async () => {
    ...
    try {
      ...
      // ⚠️ Note: Adjust this API endpoint to match your Aptitude backend route if different!
      const res = await api.post('/ai/generate-question', { 
        userId, 
        topic: formattedCategory, // Changed from company to topic/category
        difficulty: "Medium",
        type: "aptitude" 
      });
  ...
  const handleFinalSubmit = async () => {
    ...
    // Calculate score locally for DB sync
    updatedBank.forEach((q) => {
      if (q.selectedOption) {
        attempted++;
    ...
    const finalAccuracy = attempted > 0 ? Math.round((score / attempted) * 100) : 0;

    // 🔥 NEW REQUIREMENT: Send REAL score to backend for CompanyPrep collection
    if (attemptId) {
      try {
        const token = localStorage.getItem("token");
        const API_URL = import.meta.env.VITE_API_URL || "http://localhost:5000/api";
        await axios.post(`${API_URL}/company-prep/submit`, {
          attemptId,
          score: finalAccuracy
        }, {
          headers: { Authorization: `Bearer ${token}` }
        });
        console.log("✅ Score updated in Collection successfully");
      } catch (err) {
      ...
      if (q.selectedOption) {
        try {
           ...
           const res = await api.post('/ai/evaluate-solution', {
            ...
            correctAnswer: q.raw.correctAnswer, 
            questionTitle: q.raw.title, 
    ...
    setQuestionsBank(updatedBank);
    setFinalScoreStats({ attempted, score, accuracy: finalAccuracy });
      ...
      <div className="bg-white sticky top-0 z-40 shadow-sm border-b border-slate-100">
        <div className="max-w-[1600px] w-full mx-auto px-6 py-4 flex justify-between items-center">
          <div className="flex items-center gap-4">
            {/* 🔥 FIXED: Navigates back to the main Aptitude page */}
            <button onClick={() => navigate('/student/aptitude')} className="w-10 h-10 bg-slate-50 rounded-full flex items-center justify-center text-slate-500 hover:text-[#5cbdb9] transition-all border border-slate-200">
              <i className="fas fa-arrow-left"></i>
            </button>
            <h1 className="text-xl font-black text-slate-800 uppercase tracking-tight">
              {formattedCategory} <span className="text-[#5cbdb9]">Assessment</span>
            </h1>
          </div>
            ...
            {loading ? (
              ...
              showPostSubmitReview ? (
                <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="flex justify-between items-center mb-8 bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100">
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mt-1">Assessment Feedback</p>
                    </div>
                    ...
                    {questionsBank.map((q, idx) => (
                      <div key={idx} className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100">
                        ...
                        <div className="bg-slate-50 p-5 rounded-xl text-sm text-slate-600 border border-slate-200">
                          <span className="font-black text-[#5cbdb9] uppercase text-[10px] block mb-2">Explanation</span>
                ...
                <div className="bg-white p-12 rounded-[2.5rem] shadow-sm border border-slate-100 text-center animate-in zoom-in duration-500 h-[70vh] flex flex-col justify-center">
                  ...
                  <p className="text-slate-500 mb-10 font-medium">You have successfully finished the {formattedCategory} test block.</p>
                  ...
                  <div className="flex justify-center gap-4">
                    ...
                    {/* 🔥 FIXED: Now explicitly navigates to dashboard */}
              ...
              <div className="bg-white p-8 sm:p-10 rounded-[2.5rem] shadow-sm border border-slate-100 select-none animate-in fade-in" onCopy={(e) => { e.preventDefault(); alert("Copying is disabled."); }}>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 border-b border-slate-100 pb-6 gap-4">
                  <span className="bg-[#5cbdb9]/10 text-[#5cbdb9] px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest">
                    Topic: {currentQ.raw.title || formattedCategory}
                  </span>
